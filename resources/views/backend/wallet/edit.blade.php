        ...
        <div class="col">
            <div class="card">
                <div class="d-flex justify-content-between border-bottom card-header">
                    <h5 class="card-title">Edit Wallets</h5>
                    <a href="{{ route('wallets.index') }}">
                        <h6 class="mt-3 btn btn-primary">All Wallet</h6>
                    </a>
                </div>
                <div class="card-body mt-3">
                    <form class="form-horizontal" action="{{ route('wallets.update',$wallet->id) }}" method="post">
                        @csrf
                        @method('put')
                        <div class="row mb-4">
                            ...
                            <div class="col-md-9">
                               <select name="user_id" class="form-control user" id="">
                                @forelse ($users as $user)
                                    <option value="{{ $user->id }}" {{ $wallet->id == $user->id ? 'selected': '' }}>{{ $user->name }} <span>(</span> {{$user->email}} <span>)</span></option>
                                @empty
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-9">
                                <input min="1" class="form-control" value="{{ $wallet->balance }}" name="balance" id="balance"
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-9 ">
                                <select name="status" id="" class="form-control">
                                    <option value="1" {{ $wallet->status == 1 ? 'selected': '' }}>Active</option>
                                    <option value="0" {{ $wallet->status == 0 ? 'selected': '' }}>Inactive</option>
                                </select>
                            </div>
                        </div>
