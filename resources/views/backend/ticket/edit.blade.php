        ...
        <div class="col">
            <div class="card">
                <div class="d-flex justify-content-between border-bottom card-header">
                    <h5 class="card-title">Edit tickets</h5>
                    <a href="{{ route('tickets.index') }}">
                        <h6 class="mt-3 btn btn-primary">All ticket</h6>
                    </a>
                </div>
                <div class="card-body mt-3">
                    <form class="form-horizontal" action="{{ route('tickets.update',$ticket->id) }}" method="post">
                        @csrf
                        @method('put')
                        <div class="row mb-4">
                            ...
                            <div class="col-md-9">
                               <select name="user_id" class="form-control user" id="">
                                @forelse ($users as $user)
                                    <option value="{{ $user->id }}" {{ $ticket->id == $user->id ? 'selected': '' }}>{{ $user->name }} <span>(</span> {{$user->email}} <span>)</span></option>
                                @empty
                            ...
                            <div class="col-md-9">
                               <select name="train_service_id" class="form-control train_service" id="">
                                @forelse ($train_services as $train_service)
                                    <option value="{{ $train_service->id }}" {{ $ticket->id == $train_service->id ? 'selected': '' }}>{{ $train_service->name }}</option>
                                @empty
                            ...
                            <div class="col-md-9">
                               <select name="station_id" class="form-control station" id="">
                                @forelse ($stations as $station)
                                    <option value="{{ $station->id }}" {{ $ticket->id == $station->id ? 'selected': '' }}>{{ $station->name }} <span>(</span> {{$user->email}} <span>)</span></option>
                                @empty
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-9">
                                <input min="1" class="form-control" value="{{  $ticket->departure_time  }}" name="departure_time" id="departure_time"
                                     type="date" />
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-9">
                                <input min="1" class="form-control" value="{{ $ticket->price }}" name="price" id="price"
